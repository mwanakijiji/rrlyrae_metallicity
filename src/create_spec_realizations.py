...
simulating Guassian noise. Then it runs Ken's bkgrnd routine to determine the normalization and finally creates the normalized spectra.
...
def generate_realizations(spec_name,outdir,num):
    '''
    Calculates a Number of Realizations of a given spectrum using Guassian Errorbars
...
def create_spec_realizations_main(input_list,outdir,num=100,verb=False):
    ...
    #Run bckgrnd
    bkgrnd = Popen(["../bin/bkgrnd","--smooth 20","--sismoo 1", "--no-plot", "{}".format(bkg_input_file)],stdout=PIPE,stderr=PIPE)
...
if __name__ == '__main__':
    ...
    parser.add_argument('input_list',help='List of spetra to process.')
